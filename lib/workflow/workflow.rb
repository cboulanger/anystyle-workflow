...
module Workflow
  class Workflow
    ...
    class << self
      ...
      def create_gold_csljson
        anystyle = Datamining::AnyStyle.new
        files = Dir.glob('data/0-gold-anystyle-xml/*.xml').map(&:untaint)
        files.each do |file_path|
          file_name = File.basename file_path, '.xml'
          xml = File.read file_path
          csl = anystyle.xml_to_csl xml
          File.write("data/0-gold-csl/#{file_name}.json", JSON.pretty_generate(csl))
        end
      end

      def create_gold_json
        anystyle = Datamining::AnyStyle.new
        files = Dir.glob('data/0-gold-anystyle-xml/*.xml').map(&:untaint)
        files.each do |file_path|
          file_name = File.basename file_path, '.xml'
          xml = File.read file_path
          hash = anystyle.xml_to_hash xml
          File.write("data/0-gold-anystyle-json/#{file_name}.json", JSON.pretty_generate(hash))
        end
      end
      ...
      def extraction_stats
        ...
        files.each do |file_path|
          ...
          # gold files
          gf_path = "data/0-gold-csl/#{file_name}.json"
